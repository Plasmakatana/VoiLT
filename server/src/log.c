#include "log.h"
static void sanitize_for_long(char *str, size_t max_len){
  if(!str) return;
...
void log_access(const char *client_ip,const char *method, const char *uri,int status, long response_size){
  pthread_mutex_lock(&log_mutex);
  char timestamp[32];
  get_timestamp(timestamp,sizeof(timestamp));
  if(log_file!=NULL){
    char safe_method[32], safe_uri[512];
    strncpy(safe_method,method ? method:"-",sizeof(safe_method));
    strncpy(safe_uri,uri ? uri:"-",sizeof(safe_uri));
    safe_method[sizeof(safe_method)-1]='\0';
    safe_uri[sizeof(safe_uri)-1]='\0';
    sanitize_for_long(safe_method,sizeof(safe_method));
    sanitize_for_long(safe_uri,sizeof(safe_uri));
