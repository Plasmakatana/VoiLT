...
static int check_rate_limit(const char *client_ip) {
    ...
    if (entry->ban_until > 0 && now < entry->ban_until) {
        ...
        LOG_WARN("Banned IP %s attempted connection (ban expires in %ld seconds)",client_ip, entry->ban_until - now);
        return 0;
    }
    ...
    if (entry->connection_count >= MAX_CONCURRENT_CONNECTIONS_PER_IP) {
        pthread_mutex_unlock(&rate_limit_mutex);
        LOG_WARN("Too many concurrent connections from IP %s: %d",client_ip, entry->connection_count);
        return 0;
    }
    ...
    if (entry->request_count > RATE_LIMIT_MAX_REQUESTS) {
        ...
        if (entry->violation_count >= MAX_VIOLATIONS_BEFORE_BAN) {
            ...
            LOG_WARN("IP %s banned for %d seconds after %d violations",client_ip, BAN_DURATION, entry->violation_count);
        }

        pthread_mutex_unlock(&rate_limit_mutex);
        LOG_WARN("Rate limit exceeded for IP %s: %d requests in window (violation #%d)",client_ip, entry->request_count, entry->violation_count);
        return 0;
    }
...
static void decrement_connection_count(const char *client_ip) {
    ...
    if (entry->ip[0] != '\0' && strcmp(entry->ip, client_ip) == 0) {
        if (entry->connection_count > 0) {
            entry->connection_count--;
            LOG_DEBUG("Decremented connection count for IP %s: %d",client_ip, entry->connection_count);
        }
    }
    ...
    for (int i = 0; i < worker->client_count; i++) {
        if (worker->clients[i].timer_fd == timer_fd) {
            ...
            if (worker->clients[i].bytes_received > 0 && 
                ...
                LOG_WARN("Slow loris attack detected from %s: incomplete request after %ld seconds",worker->clients[i].client_ip, now - worker->clients[i].connection_start);
                worker_remove_client(worker, worker->clients[i].fd);
                break;
            }

            if (now - worker->clients[i].last_activity >= worker->keep_alive_timeout) {
                LOG_INFO("Client timeout: fd=%d, ip=%s, idle=%lds",worker->clients[i].fd, worker->clients[i].client_ip,
...
void worker_handle_connection(worker_t *worker, int client_fd) {
    ...
    if (getpeername(client_fd, (struct sockaddr*)&client_addr, &addr_len) == 0) {
        inet_ntop(AF_INET, &client_addr.sin_addr, 
                  worker->clients[worker->client_count].client_ip,INET_ADDRSTRLEN);
        LOG_INFO("Accepted connection: fd=%d, ip=%s, port=%d, clients=%d",client_fd, worker->clients[worker->client_count].client_ip,ntohs(client_addr.sin_port), worker->client_count + 1);
    } else {
    ...
    while ((bytes_read = recv(client_fd, client->buffer + total_read, BUFFER_SIZE - total_read - 1, 0)) > 0) {
        ...
        if (bytes_read == 1 && client->bytes_received > 100) {
            ...
            if ((now - client->connection_start) > 5) {
                LOG_WARN("Potential slow loris attack from %s: %d single-byte reads",client->client_ip, client->bytes_received);
...
void worker_run(worker_t *worker) {
    ...
    LOG_INFO("Worker %d entering main loop: is_running=%d, shutdown_requested=%d, worker_shutdown_requested=%d",worker->cpu_id, worker->is_running, shutdown_requested, worker_shutdown_requested);
        ...
        for (int i = 0; i < nfds; i++) {
            ...
            if (fd == worker->server_fd && (event_flags & EPOLLIN)) {
                ...
                while (accepted < max_accept_per_cycle) {
                    int client_fd = accept4(worker->server_fd,(struct sockaddr*)&client_addr,&addr_len,SOCK_NONBLOCK);
